...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        with connections['mira'].cursor() as cursor:
            sql = f"""
                SELECT id, cspec, spec_name from catadmission where yr = 2025
        ...
        admissions_by_id = {i['id']: i for i in admissions}
        ...
        for item in planlines:
            lines = item.planlines
            tmp = []

            for i in item.additional_info.all():
                if i.type == 'software':
                    for app in i.value:
            ...
            data.append({
                'Дисциплина': lines.dis,
                'Приложения': ', '.join(set(tmp)) if len(tmp) > 0 else '',
                'Специальность': admissions_by_id.get(item.cadmission, {}).get('spec_name', ''),
                'Наименование': admissions_by_id.get(item.cadmission, {}).get('name', ''),
            })
