  ...
  let data = _(typesList)
    .map(item => {
      ...
      const itemWorks = _(works.value)
        ...
        .map(x => {
          const work = _(indPlan.value.works).filter(y => {
            return y.work !== null && y.work.id === x.id
          }).value();
          if (x.is_multiple) {
            if (work.length > 0) {
...
const typesList = computed(() => {
  return _(works.value).map(x => x.type).uniq()
})
...
async function toggleWork(work_id: number, plan_work_id: number) {
  if ((plan_work_id || 0) > 0) {
    await deleteWork(plan_work_id)
  } else {
    const r = await api.post(`/api/planwork/`, {
      work_id: work_id,
      plan_id: indPlan.value.plan.id,
    });
  }
  await fetchIndPlan();
}
...
async function updateWork() {
  if (currentWork.value) {
    const r = await api.put(`/api/planwork/${currentWork.value.id}/`, {
      additional_info: currentWork.value.additional_info.toString()
    });
  }
...
async function fetchWorks() {
  let r = await api.get(`/api/indplan/${props.id}/works/`);
  works.value = r.data;
}
</script>
...
<template>
  <layout-h-c-f v-if="indPlan !== undefined && works !== undefined">
    <template #header>
      <div
        style="display: grid; grid-template-columns: auto auto auto; gap: 8px; margin: 8px; justify-content: space-between; align-items: center">
        <q-btn label="Назад" icon="mdi-arrow-left" to="/ind_plan/"/>

        <div
          style="display: grid; grid-template-columns: auto auto auto auto; gap: 8px; justify-content: center; align-items: center">
          <span style="font-size: 15px">{{
              indPlan.plan.user_created.last_name
            }} {{ indPlan.plan.user_created.first_name }} {{
              indPlan.plan.user_created.middle_name
            }}, {{ indPlan.plan.additional_info.doljn }}, ставка {{
              indPlan.plan.additional_info.rate
            }}, {{ indPlan.plan.year }} год</span>
          <q-badge
        ...
        <q-list separator>
          ...
          >
            ...
            <q-list style="padding-left: 15px"  >
              <q-item v-for="work in row.works" :style="{'background-color': work.color}" separator>
                <q-item-section style="display: grid; grid-template-columns: 400px 1fr auto; align-items: center">
                  <span>{{ work.name }}</span>
                  <q-input outlined label="Уточнение по работе" :disable="!work.to_done" />
                    <q-checkbox
                      v-model="work.to_done"
                      @click="toggleWork(work.id, work.plan_work)"
                    />
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <q-card style="min-width: 500px">
          ...
          >
            ...
            <div v-for="comment in indPlan.plan.comments" style="display: flex; flex-direction: column">
              ...
              <span style="align-self: end; color: dimgrey" v-if="comment.author !== null && comment.date !== null"> - {{
                  comment.author.last_name
                }} {{ comment.author.first_name.substring(0, 1) }}.{{
                  comment.author.middle_name.substring(0, 1)
                }}., {{ dayjs(comment.date).format('DD.MM.YYYY, HH:mm') }}</span>
              <q-separator/>
            </div>
          </q-card-section>
